...
class PP(nn.Module):
    def __init__(self, ):
        ...
        # self.mull = nn.Parameter(torch.Tensor([0.3]).repeat(7))
        self.stages = 6
...
class HTCUNet(nn.Module):
    def __init__(self, in_c=3, out_c=3, n_feat=32, scale_unetfeats=16, scale_orsnetfeats=16, num_cab=8, kernel_size=3,
        ...
        super(HTCUNet, self).__init__()
        self.n_input = int(cs_ratio * 1024)
        self.block_size = block_size

        self.Phi = nn.Parameter(
            init.xavier_normal_(torch.Tensor(np.ceil(cs_ratio * 0.01 * 1024.).astype(np.int), 1024)))
        act = nn.PReLU()
    ...
    def forward(self, img):
        ...
        # pp_0 = Phix.view(b, l, -1).permute(0, 2, 1).contiguous()
        # pp_0 = F.fold(pp_0, output_size=(w, h), kernel_size=self.block_size, stride=self.block_size).contiguous()
